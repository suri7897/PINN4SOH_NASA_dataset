...
class TJUdata(DF):
    ...
    def read_one_batch(self,batch):
        '''
        Read a batch of csv files
        :param batch: int[1,2,3]; optional[1,2,3]
        :return: DataFrame
        '''
        assert batch in [1,2,3], 'batch must be in {}'.format([1,2,3])
        root = os.path.join(self.root,self.batchs[batch-1])
...
class NASAdata(DF):
    def __init__(self, root='NASA_dataset/battery_splited', args=None):
        ...
        if self.normalization:
            ...
            self.nominal_capacities = {batch: None for batch in self.batchs}

    def read_one_batch(self, batch):
        assert batch in [1,2,3,4,5,6,7,8,9], 'batch must be in {}'.format([1,2,3,4,5,6,7,8,9])
        root = os.path.join(self.root,self.batchs[batch-1])
        file_list = os.listdir(root)
        df = pd.DataFrame()
        path_list = []
    ...
    #     return self.load_all_battery(path_list=specific_path_list, nominal_capacity=normal_capacity)
    def read_all(self, specific_path_list):
        if not specific_path_list:
            print("Warning: specific_path_list is empty. Skipping this batch.")
            return {'train_2': [], 'valid_2': [], 'test_3': []}
        normal_capacity = None
        for batch_name, ids in self.batchs.items():
            for battery_id in ids:
                if f'B{battery_id:04d}' in specific_path_list[0]:
                    normal_capacity = self.nominal_capacities if isinstance(self.nominal_capacities, float) else self.nominal_capacities[batch_name]
                    break
...
if __name__ == '__main__':
    ...
    def get_args():
        ...
        parser.add_argument('--data',type=str,default='MIT',help='XJTU, HUST, MIT, TJU, NASA')
    ...
    # nasa = NASAdata(args=args)
    # nasa.read_one_batch(batch=1)
    # loader = nasa.read_all()
