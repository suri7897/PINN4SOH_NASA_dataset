...
if __name__ == '__main__':
    ...
    if cnn:
        ...
        pdf = PdfPages(os.path.join(savedir,'Figure 4b.pdf'))
        with plt.rc_context({'text.usetex': False}):
            fig, axs = plt.subplots(4,4,figsize=(2.3*4,1.8*3),dpi=200)
            colors = ['#b8dff2','#abeadb','#ffb2b4']
            count = 0
            for data in ['NASA','XJTU','TJU','MIT','HUST']:
                if data == 'XJTU':
                    batches = [0,1,2,3,4,5]
                elif data == 'TJU':
                    batches = [0,1,2]
            ...
            legend_labels = ['Ours','MLP','CNN','Mean','Mean $\\pm$ Std']
            # axs[3, 3].legend(handles=boxs,labels=legend_labes, loc=[0.2, 0],
            #                 handlelength=4,
            #                 handleheight=2.5,
            #                 fontsize=8)

            plt.tight_layout()
            plt.savefig(os.path.join(savedir,'Figure 4b.png'),format='png')
            pdf.savefig()
            pdf.close()
            plt.show()
    else :
        print("Plotting Figure 4b without CNN...")
        pdf = PdfPages(os.path.join(savedir,'Figure 4b_wo_CNN.pdf'))
        with plt.rc_context({'text.usetex': False}):
            fig, axs = plt.subplots(4,4,figsize=(2.3*4,1.8*3),dpi=200)
            ...
            boxs.append(mean_line)
            boxs.append(std_line)
            legend_labels = ['Ours','MLP','Mean','Mean $\\pm$ Std']
            # axs[3, 3].legend(handles=boxs,labels=legend_labes, loc=[0.2, 0],
            #                 handlelength=4,
            #                 handleheight=2.5,
            #                 fontsize=8)
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(savedir,'Figure 4b_wo_CNN.png'),format='png')
            pdf.savefig()
            pdf.close()
            plt.show()
