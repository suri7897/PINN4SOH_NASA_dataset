...
class Results:
    def __init__(self,root='../results of reviewer/PINN/NASA results/',gap=0.02):
        self.root = root
    ...
    def parser_log(self):
        ...
        if '.csv' in line1:
            line = line1[1:-2]
            line_list = line.replace('data/NASA data/', '').replace('.csv','').replace('\'','').split(', ')
...
if __name__ == '__main__':
    ...
    root = '../results of reviewer/PINN/NASA results/'
    writer = pd.ExcelWriter('../results of reviewer/PINN-NASA-results.xlsx')
    batch = 0
